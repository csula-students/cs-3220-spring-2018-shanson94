...
function main() {
	...
	const initialState = {
		...
		story: [{
			name: 'The Art of The Pickaxe',
			description: 'After mining ore with your bare hands, you have come to understand how to use a pickaxe',
			triggerdAt: '10',
			state: 'hidden'
		}, {
			name: 'Goblins Come',
			description: 'After word of your ore spreads, goblins flock to your mines',
			triggerdAt: '50',
			state: 'hidden'
		}, {
			name: 'Goblins Make Machines',
			description: 'The goblins, growing lazy of mining, build machine miners to do all the work',
			triggedAt: '500',
			state: 'hidden'
		}]
	};
...
class Generator {
  ...
  */
	getCost() {
		var rt = Math.pow(1 + _constants2.default.growthRatio, this.quantity);
	...
	return class StoryBookComponent extends window.HTMLElement {
		...
		handleStateChange(newState) {
			...
			for (var i = 0; i < store.state.story.length; i++) {
				const story = new _story2.default(store.state.story[i]);
